...
void	draw_wall(t_wall *wall, t_info *info, int x)
{
	...
	t_num = info->img.texnum;
	wall->texx = (int)(wall->wallx * (double)info->img.w[t_num]);
	if (wall->side == 0 && wall->rdirx > 0)
		wall->texx = info->img.w[t_num] - wall->texx - 1;
	if (wall->side == 1 && wall->rdiry < 0)
		wall->texx = info->img.w[t_num] - wall->texx - 1;
	wall->step = 1.0 * info->img.h[t_num] / wall->l_height;
	texpos = (wall->draw_s - info->s_h / 2 + wall->l_height / 2) * wall->step;
	y = wall->draw_s;
	while (y < wall->draw_e)
	{
		wall->texy = (int)texpos;
		if (wall->texy > (info->img.h[t_num] - 1))
			wall->texy = (info->img.h[t_num] -1);
		texpos += wall->step;
		color = info->tex[t_num][info->img.r_w[t_num] * wall->texy + wall->texx];
		info->buf[y][x] = color;
		y++;
	}
}
...
void	draw_image(t_info *info, t_img *img)
{
	...
	while (y < info->s_h)
	{
		...
		//	img->data[y * info->s_w + x] = info->buf[y][x];
			img->data[y * (img->l_s / 4) + x] = info->buf[y][x];
			x++;
		}
		y++;
	}
